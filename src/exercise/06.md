...
In React, we attach event handlers to a JSX component by specifying them as an attribute as shown below. We can specify the event using the format: `on<event-name>`.  
  
The event handler in this case is a function called `handleClick`.
   
We do not call the function ourselves and so there are no brackets in the following code after `handleClick`. We just pass the function to React so that it can call the event handler when the event has been triggered.
```html
<div onClick={handleClick}>
   Hello World
</div>
```
When it calls the event handler, React passes a event object to it. This event object is not the normal event object thrown by the browser. It is a React created object wrapper over the event object thrown by the browser but for almost all purposes, it behaves similarly.

If we anyhow need to access the actual browser event object directly, we can use `event.nativeEvent` to directly access the native browser event object.

We can get the HTMLElement which is throwing the error by using `event.target`. If the target is a form element and it contains multiple inputs, we get an array of the values inside it which we can access using `event.target[i].value`. But this is not a good way to access the values since the orders of inputs may change later.

If we set the `name` or `id` attribute of the `input` element, we can access the values using that with `event.target.elements.usernameInput.value` where `usernameInput` is the `id` or `name` value. The `id` value takes precedence over `name` if both are set.

### Controlled Components
Components whose value is controlled by React are known as controlled components. In the below example, the input form is controlled by React as its value is changed by React using a React state variable called `username`. Whenever the value changes, React calls `handleChange` which changes the value and the new value is updated in the input text box using `username`. Both the value prop as well as the event handler is required to make a controlled component.
