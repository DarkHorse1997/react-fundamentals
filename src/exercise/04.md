...
## 📝 Your Notes
* We make Javascript functions that return some JSX to create reusable components. We can call that fucntion wherever needed in our JSX components using interpolation.
  ```jsx
  ...
  ```
* `React.createElement`: We can pass a resuable component function as the first argument. It creates a DOM node according to the code in the component passed and the object passed as the second argument.
  ```jsx
  const message = ({children}) => {
        return (
          <div>{children}</div>
  ...
  ```
  * A React component is created when we use `React.createElement(message)`. Just calling the message() function as in the previous example doesn't create a component. We can notice this in the **Components** Tab of React DevTools.

* React component names should start with a Capital Letter according to the JSX specification. Thus if the function name starts with a capital letter it is created as a React component by babel, i.e., babel passes it to `React.createElement()` as a reference rather than a string.
  ```jsx
  const Message = ({children}) => {
      return (
        <div>{children}</div>
  ...
  ```
* We can do prop validation using Proptypes.
* We can use `React.Fragment` to create an empty JSX container.
