...
* `document.createElement()` : Used to create a DOM element.
* `element.setAttribute(attributeName, attributeValue` : Used to set an attribute of a DOM element.
* `element.textContent` : Used to set the text inside the DOM element.
* `element.className` : Used to add a classname to the DOM element.
* `ParentNode.append()` : Used to insert the DOM element in the HTML Document. 
